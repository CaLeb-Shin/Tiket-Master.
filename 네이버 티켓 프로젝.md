...
### Phase 3: Admin UI 🔄
- [ ] `naver_order_screen.dart` — 주문 입력/관리 화면
  - 상단: 요약 카드 (전체/확정/취소)
  - 주문 입력 폼 (네이버주문번호, 구매자명, 전화, 등급, 수량, 날짜, 메모)
  - 주문 목록 테이블 (상태 필터, 검색, 취소 버튼, 티켓URL 복사)
  - 주문별 MobileTicket 확장 표시 (입장번호, accessToken URL)
- [ ] 라우터: `/events/:eventId/naver-orders` 추가
- [ ] 사이드바: 이벤트 드롭다운 메뉴에 '네이버 주문' 항목 추가

### Phase 4: 모바일 티켓 페이지 ⬜
- [ ] `mobile_ticket_page.dart` — 공개 구매자용 페이지
- [ ] 라우터 `/m/:accessToken` 추가
- [ ] QR 자동갱신 (2분 JWT)
- [ ] 좌석 공개 전/후 분기 표시
...
| 2026-03-02 | Phase 3 Admin UI 구현 시작 | 🔄 |
