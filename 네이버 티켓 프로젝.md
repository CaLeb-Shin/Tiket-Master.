...
### Phase 4: 모바일 티켓 페이지 🔄
- [ ] `mobile_ticket_page.dart` — 공개 구매자용 페이지 (비로그인)
  - 공연 포스터 + 제목 + 일시 + 장소
  - 등급 배지 + 입장번호
  - 좌석공개 전: "좌석 미공개" + 카운트다운
  - 좌석공개 후: 층/구역/열/좌석번호
  - QR 코드 (2분 자동갱신, JWT)
- [ ] 라우터 `/m/:accessToken` 추가 (auth guard 밖에 배치)
- [ ] getMobileTicketByToken + issueMobileQrToken 연동
...
| 2026-03-02 | Phase 4 모바일 티켓 페이지 구현 시작 | 🔄 |
