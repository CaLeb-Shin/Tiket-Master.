...
async function generateShortToken(payload, type) {
...
const DEVICE_CACHE_TTL_MS = 5 * 60 * 1000; // 5분
// 취소 수수료 규정 (실제 서비스 기준)
// - 예매 후 7일 이내: 무료취소
...
/** 스캐너 기기 검증 (5분 캐시). 결과: { valid, reason?, message? } */
async function validateDevice(deviceId, uid) {
    // 캐시 확인
    const cached = deviceCache.get(deviceId);
    if (cached && Date.now() - cached.ts < DEVICE_CACHE_TTL_MS) {
        if (uid && cached.ownerUid !== uid)
            return { valid: false, reason: "기기 소유자 불일치", message: "현재 계정으로 승인된 스캐너 기기가 아닙니다" };
        if (cached.blocked)
            return { valid: false, reason: "차단 기기", message: "차단된 스캐너 기기입니다" };
        if (!cached.approved)
            return { valid: false, reason: "미승인 기기", message: "승인 대기 중인 스캐너 기기입니다" };
        // lastSeenAt 쓰로틀링
        const now = Date.now();
        const lastSeen = lastSeenCache.get(deviceId) || 0;
        if (now - lastSeen > LAST_SEEN_THROTTLE_MS) {
            db.collection("scannerDevices").doc(deviceId).set({ lastSeenAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(() => { });
            lastSeenCache.set(deviceId, now);
        }
        return { valid: true };
    }
    // Firestore 조회
    const doc = await db.collection("scannerDevices").doc(deviceId).get();
    const dev = doc.data();
    if (!doc.exists || !dev)
        return { valid: false, reason: "등록되지 않은 기기", message: "등록되지 않은 스캐너 기기입니다. 관리자 승인 후 사용해 주세요" };
    // 캐시 저장
    deviceCache.set(deviceId, { ownerUid: dev.ownerUid, approved: dev.approved === true, blocked: dev.blocked === true, ts: Date.now() });
    if (uid && dev.ownerUid !== uid)
        return { valid: false, reason: "기기 소유자 불일치", message: "현재 계정으로 승인된 스캐너 기기가 아닙니다" };
    if (dev.blocked === true)
        return { valid: false, reason: "차단 기기", message: "차단된 스캐너 기기입니다" };
    if (dev.approved !== true)
        return { valid: false, reason: "미승인 기기", message: "승인 대기 중인 스캐너 기기입니다" };
    // lastSeenAt
    const now2 = Date.now();
    db.collection("scannerDevices").doc(deviceId).set({ lastSeenAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(() => { });
    lastSeenCache.set(deviceId, now2);
    return { valid: true };
}
...
exports.createOrder = functions.https.onCall(async (data, context) => {
    ...
    const referralCode = typeof data?.referralCode === "string" && data.referralCode.trim().length > 0
...
// ============================================================
exports.verifyAndCheckIn = functions
    ...
    if (!scannerDeviceId) {
        return { success: false, result: "notAllowedDevice", title: "승인되지 않은 기기", message: "승인된 스캐너 기기에서만 입장 체크가 가능합니다" };
    }
    // ── QR 포맷 판별 (short token vs JWT) ──
    const rawQr = (ticketId === "m" || ticketId === "mm")
        ? `${ticketId}:${qrToken}`
        : (qrToken || ticketId || "").trim();
    const isShortToken = rawQr.startsWith("m:") || rawQr.startsWith("mm:");
    ...
    const [scannerUid, shortTokenDoc, deviceStatus] = await Promise.all([
        ...
        isDemoDevice ? Promise.resolve({ valid: true }) : validateDevice(scannerDeviceId, context?.auth?.uid),
    ]);
    ...
    if (!deviceStatus.valid) {
        logCheckin(ticketId || "unknown", actorStaffId, "notAllowedDevice", deviceStatus.reason, { stage: checkinStage, scannerDeviceId });
        return { success: false, result: "notAllowedDevice", message: deviceStatus.message };
    }
    // 모바일 티켓(mt_) 분기
    ...
    if (qrToken !== "__short__") {
        ...
        try {
            ...
            logCheckin(ticketId, actorStaffId, result, error.message, { stage: checkinStage, scannerDeviceId });
            return { success: false, result, title: result === "expired" ? "QR 만료" : "잘못된 QR", message: result === "expired" ? "QR이 만료되었습니다" : "잘못된 QR입니다" };
        }
        if (decoded.ticketId !== actualTicketId) {
            logCheckin(actualTicketId, actorStaffId, "invalidTicket", "티켓 ID 불일치", { stage: checkinStage, scannerDeviceId });
            return { success: false, result: "invalidTicket", title: "잘못된 티켓", message: "잘못된 티켓입니다" };
        }
    }
    // 트랜잭션으로 체크인 처리
    return db.runTransaction(async (transaction) => {
 ...
 */
async function addMileageInternal(userId, amount, type, reason) {
    ...
    functions.logger.info(`마일리지 내부 ${amount > 0 ? "적립" : "차감"}: user=${userId}, amount=${amount}, type=${type}, balance=${newBalance}`);
}
// ============================================================
...
// ============================================================
exports.verifyAndCheckInGroup = functions
    .runWith({ minInstances: 1 })
    .https.onCall(async (data, context) => {
    ...
    // QR 포맷 판별
    const rawQr = orderId === "mg" ? `mg:${qrToken}` : (qrToken || orderId || "").trim();
...
function requireBotRequestAuth(req, res) {
    let apiKey;
    try {
...
function httpStatusFromFunctionsErrorCode(code) {
    switch (code) {
...
function sendHttpError(res, error, logLabel) {
...
async function createNaverOrderInternal(raw, options = {}) {
    let input;
    try {
        input = (0, naver_ticket_logic_1.parseNaverOrderCreateInput)(raw);
    }
    catch (error) {
        if (error instanceof naver_ticket_logic_1.NaverTicketLogicError) {
...
async function getNaverOrderDocByNaverOrderId(naverOrderId) {
...
async function cancelNaverOrderInternal(orderDoc, options = {}) {
    const order = orderDoc.data();
    if (!order) {
        throw new functions.https.HttpsError("not-found", "주문을 찾을 수 없습니다");
    }
    try {
        const policy = (0, naver_ticket_logic_1.evaluateCancelOrderStatus)(order.status, options.allowAlreadyCancelled === true);
 ...
 */
exports.createNaverOrder = functions.https.onCall(async (data, context) => {
    await assertAdmin(context?.auth?.uid);
 ...
 */
exports.cancelNaverOrder = functions.https.onCall(async (data, context) => {
    await assertAdmin(context?.auth?.uid);
    const { orderId } = data;
    if (!orderId) {
 ...
 */
exports.claimNaverOrder = functions.https.onCall(async (data, context) => {
    const userId = context?.auth?.uid;
    if (!userId) {
        throw new functions.https.HttpsError("unauthenticated", "로그인이 필요합니다");
    }
    ...
    const phoneMatches = buyerPhoneDigits.length >= 7
 ...
 */
exports.autoClaimNaverOrdersByPhone = functions.https.onCall(async (data, context) => {
    const userId = context?.auth?.uid;
    if (!userId) {
        throw new functions.https.HttpsError("unauthenticated", "로그인이 필요합니다");
    }
 ...
 */
exports.issueMobileQrToken = functions.https.onCall(async (data) => {
    ...
    if (!isEventRevealed(event)) {
        throw new functions.https.HttpsError("failed-precondition", "공연 시작 2시간 전부터 입장 QR을 발급할 수 있습니다");
    }
    // Short token 생성
    const shortToken = await generateShortToken({
        ticketId,
...
exports.getTicketOgMeta = functions.https.onRequest(async (req, res) => {
    // CORS 허용
    ...
    if (!token) {
        res.status(400).json({ error: "token 필요" });
        return;
    }
 ...
 */
exports.getMobileTicketByToken = functions.https.onCall(async (data) => {
...
// ============================================================
exports.setRecipientName = functions.https.onCall(async (data) => {
 ...
 */
exports.revealSeatsNow = functions.https.onCall(async (data, context) => {
    await assertAdmin(context?.auth?.uid);
    const { eventId } = data;
    if (!eventId) {
        throw new functions.https.HttpsError("invalid-argument", "eventId가 필요합니다");
 ...
 */
exports.reassignTicketSeat = functions.https.onCall(async (data, context) => {
    await assertAdmin(context?.auth?.uid);
 ...
 */
exports.createNaverOrderHttp = functions.https.onRequest(async (req, res) => {
    // CORS
    res.set("Access-Control-Allow-Origin", "*");
    if (req.method === "OPTIONS") {
 ...
 */
exports.getPendingSmsHttp = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    if (req.method === "OPTIONS") {
        res.status(204).send("");
        return;
 ...
 */
exports.markSmsSentHttp = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    if (req.method === "OPTIONS") {
        res.status(204).send("");
        return;
...
// ============================================================
exports.healthCheckMobileTicket = functions.pubsub
...
// ═══════════════════════════════════════════════════════════════════════════
exports.holdSeat = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "로그인 필요");
    }
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (seat.heldBy && seat.heldBy !== uid) {
            ...
            if (heldUntil > new Date()) {
                throw new functions.https.HttpsError("failed-precondition", "다른 사용자가 선점 중");
            }
        }
        ...
        if (myHolds.size >= MAX_HOLDS_PER_USER) {
            throw new functions.https.HttpsError("resource-exhausted", `최대 ${MAX_HOLDS_PER_USER}석까지 선점 가능`);
        }
...
// ═══════════════════════════════════════════════════════════════════════════
exports.releaseSeat = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "로그인 필요");
    }
    const uid = context.auth.uid;
    ...
    return db.runTransaction(async (tx) => {
        ...
        // 본인 홀드만 해제 가능
        if (seat.heldBy !== uid) {
            throw new functions.https.HttpsError("permission-denied", "본인 선점만 해제 가능");
        }
        tx.update(seatRef, {
...
// ═══════════════════════════════════════════════════════════════════════════
exports.releaseExpiredHolds = functions.pubsub
...
exports.confirmDesignatedSeat = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        functions.logger.info(`[DesignatedSeat] 좌석 확정: ticket=${ticketDoc.id}, seat=${seatId}, info=${result.seatInfo}`);
        res.status(200).json({
            success: true,
...
// ═══════════════════════════════════════════════════════════════════════════
exports.getDesignatedSeatInfo = functions.https.onRequest(async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method not allowed" });
        return;
    }
...
// ═══════════════════════════════════════════════════════════════════════════
exports.scheduledAutoAssignDesignated = functions.pubsub
    .schedule("every 30 minutes")
    .onRun(async () => {
    const now = admin.firestore.Timestamp.now();
    ...
    for (const [key, tickets] of groups) {
        ...
        if (availableSnap.empty) {
            functions.logger.warn(`[AutoAssign] ${seatGrade} 잔여 좌석 없음 — ${tickets.length}장 미배정 유지`);
        ...
        if (!selectedSeatDocs) {
            ...
            if (availableDocs.length < tickets.length) {
                functions.logger.warn(`[AutoAssign] ${seatGrade} 좌석 부족 (${availableDocs.length}/${tickets.length})`);
            }
    ...
    functions.logger.info(`[AutoAssign] 자동배정 완료: ${totalAssigned}장 배정, ${totalFailed}장 실패`);
    return null;
});
/** 좌석 docs를 티켓 docs에 순서대로 배정 (batch write) */
async function assignSeatsToTickets(seatDocs, ticketDocs) {
    const batch = db.batch();
    const now = admin.firestore.Timestamp.now();
...
// ═══════════════════════════════════════════════════════════════════════════
exports.changeDesignatedSeat = functions.https.onRequest(async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method not allowed" });
        return;
    }
    try {
        ...
        functions.logger.info(`[DesignatedSeat] 좌석 변경: ticket=${ticketDoc.id}, ${ticket.seatId} → ${newSeatId}, info=${result.seatInfo}`);
        res.status(200).json({
            success: true,
            seatInfo: result.seatInfo,
            seatNumber: result.seatNumber,
