export type ParsedNaverOrderCreateInput = {
  eventId: string;
  naverOrderId: string;
  buyerName: string;
  buyerPhone: string;
...
function buildPublicTicketDisplayState(
...
export function parseNaverOrderCreateInput(raw: any): ParsedNaverOrderCreateInput {
  ...
  if (!eventId || !naverOrderId || !buyerName || !buyerPhone || !seatGrade || !Number.isInteger(quantity) || quantity <= 0) {
    throw new NaverTicketLogicError(
      "invalid-argument",
      "필수 필드가 누락되었거나 수량이 올바르지 않습니다",
    );
  }
...
export function evaluateCancelOrderStatus(
  status: string | undefined,
  allowAlreadyCancelled = false,
): { alreadyCancelled: boolean; message?: string } {
  if (status === "confirmed") {
...
function buildPublicTicketDto(params: {
  ...
  return {
    ...
    buyerPhoneLast4:
      params.ticket.buyerPhone && params.ticket.buyerPhone.length >= 4
...
export function buildMobileTicketPublicPayload(params: {
  ticketId: string;
  ticket: any;
  event: any | null;
  siblingDocs?: PublicTicketDocInput[];
  now?: Date;
}) {
